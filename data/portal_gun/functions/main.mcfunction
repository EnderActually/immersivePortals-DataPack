...
execute as @a[scores={use.portal_gun=1},nbt={SelectedItemSlot:2,Inventory:[{Slot:-106b,tag:{portal:1b}}]}] at @s run function portal_gun:kill_portal
...
###Distancing
execute as @a at @s as @e[type=armor_stand,tag=portal.2,distance=1..] if score @s portal.sid = @p portal.last.sid run tag @p remove teleport2
execute as @a at @s as @e[type=armor_stand,tag=portal.1,distance=1..] if score @s portal.sid = @p portal.last.sid run tag @p remove teleport1

###Reset
scoreboard players set @a use.portal_gun 0
execute as @a unless score @s portal.sid matches 0.. run function portal_gun:player_id_setup
